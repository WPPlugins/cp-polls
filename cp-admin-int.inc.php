...
<div id="normal-sortables" class="meta-box-sortables">
 ...
 <div id="metabox_basic_settings" class="postbox" >
  ...
  <div class="inside">
       ...
       <tr><td colspan="2">Note: The following settings fields are fully available only in the commercial versions of the plugin:</td></tr>
        
       <tr valign="top">
        <th scope="row">Poll Status:</th>
        <td>
          <?php $option = $this->get_option('poll_status', ''); ?>
          <select name="poll_status">
            <option value="open"<?php if ($option == 'open' || $option == '') echo ' selected'; ?>>Open</option>            
          </select>      * Closed polls don't allow new votes. Fully available in commercial version.
        </td>
        </tr>  
                
        <tr valign="top">
        <th scope="row">Graphic layout:</th>
        <td>
          <?php $option = $this->get_option('poll_layout', 'vertical'); ?>
          <select name="poll_layout">
            <option value="horizontal"<?php if ($option == 'horizontal' || $option == '') echo ' selected'; ?>>Horizontal</option>            
          </select>      * Horizontal or Vertical bars in the reports. Fully available in commercial version.
        </td>
        </tr>         
        <tr valign="top">
        <th scope="row">Display items with zero votes:</th>
        <td>
          <?php $option = $this->get_option('poll_zero_votes', 'false'); ?>
          <select name="poll_zero_votes">
            <option value="false"<?php if ($option == 'false') echo ' selected'; ?>>No</option>            
          </select>           * Note: "Yes" is recommended for vertical layouts. Fully available in commercial version.
        </td>
        </tr> 
        <tr valign="top">
        <th scope="row">Sort results:</th>
        <td>
          <?php $option = $this->get_option('poll_sort', 'false'); ?>
          <select name="poll_sort">
            <option value="false"<?php if ($option == 'false') echo ' selected'; ?>>No</option>            
          </select>   * Sort the bars or keep the default order. Fully available in commercial version.         
        </td>
        </tr>                                  
       </table>
 ...
 <div id="metabox_basic_settings" class="postbox" >
  ...
  <div class="inside">   
     <table class="form-table">    
        ...
        - The styles can be applied into any of the CSS files of your theme or into the CSS file <em>"cp-polls\css\stylepublic.css"</em>. <br />
        - For further modifications the vote button is located at the end of the file <em>"cp-public-int.inc.php"</em>.<br />
        - For general CSS styles modifications to the form and samples <a href="http://wordpress.dwbooster.com/faq/cp-polls#q213" target="_blank">check this FAQ</a>.
        </tr>
     </table>
  </div>    
 </div> 
