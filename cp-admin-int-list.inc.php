...
if (isset($_GET['a']) && $_GET['a'] == '1')
{
    ...
    $verify_nonce = wp_verify_nonce( $_GET['rsave'], 'cfpoll_update_actions_plist');
    if (!$verify_nonce)
    {
...
else if (isset($_GET['u']) && $_GET['u'] != '')
{
    ...
    $wpdb->query( $wpdb->prepare( 'UPDATE `'.$wpdb->prefix.$this->table_items.'` SET form_name=%s WHERE id=%d', strip_tags($_GET["name"]), $_GET['u'] ) );    
...
} else if (isset($_GET['c']) && $_GET['c'] != '')
{
    ...
    $myrows = $wpdb->get_row( $wpdb->prepare("SELECT * FROM ".$wpdb->prefix.$this->table_items." WHERE id=%s", $_GET['c'])  , ARRAY_A);    
    unset($myrows["id"]);
...
{        
    $verify_nonce = wp_verify_nonce( $_POST['rsave'], 'cfpoll_update_actions_plist');
    if (!$verify_nonce)
    {
        echo 'Error: Form cannot be authenticated (nonce failed). Please contact our <a href="https://wordpress.org/support/plugin/cp-polls#new-post">support service</a> for verification and solution. Thank you.';
        return;
    ...
    update_option( 'CP_POLLS_rep_enable', sanitize_text_field($_POST["CP_POLLS_rep_enable"]));
    update_option( 'CP_POLLS_rep_days', sanitize_text_field($_POST["CP_POLLS_rep_days"]));
    update_option( 'CP_POLLS_rep_hour', sanitize_text_field($_POST["CP_POLLS_rep_hour"]));
    update_option( 'CP_POLLS_rep_emails', sanitize_text_field($_POST["CP_POLLS_rep_emails"]));
    update_option( 'CP_POLLS_fp_from_email', sanitize_text_field($_POST["CP_POLLS_fp_from_email"]));
    update_option( 'CP_POLLS_rep_subject', sanitize_text_field($_POST["CP_POLLS_rep_subject"]));
    update_option( 'CP_POLLS_rep_emailformat', sanitize_text_field($_POST["CP_POLLS_rep_emailformat"]));
    update_option( 'CP_POLLS_rep_message', sanitize_text_field($_POST["CP_POLLS_rep_message"]));
...
$nonce = wp_create_nonce( 'cfpoll_update_actions_plist' );
...
<script type="text/javascript">
 function cp_addItem()
 {
    ...
    document.location = 'options-general.php?page=<?php echo $this->menu_parameter; ?>&rsave=<?php echo $nonce; ?>&a=1&r='+Math.random()+'&name='+encodeURIComponent(calname);       
 }
 
 function cp_updateItem(id)
 {
    var calname = document.getElementById("calname_"+id).value;    
    document.location = 'options-general.php?page=<?php echo $this->menu_parameter; ?>&rsave=<?php echo $nonce; ?>&u='+id+'&r='+Math.random()+'&name='+encodeURIComponent(calname);    
 }
 
 function cp_cloneItem(id)
 {
    document.location = 'options-general.php?page=<?php echo $this->menu_parameter; ?>&rsave=<?php echo $nonce; ?>&c='+id+'&r='+Math.random();  
 } 
 ...
 function cp_deleteItem(id)
 {
    ...
    {        
        document.location = 'options-general.php?page=<?php echo $this->menu_parameter; ?>&rsave=<?php echo $nonce; ?>&d='+id+'&r='+Math.random();
    }
 }
 ...
 function cp_updateConfig()
 {
    ...
    {        
        ...
        document.location = 'options-general.php?page=<?php echo $this->menu_parameter; ?>&rsave=<?php echo $nonce; ?>&ac=st&scr='+scr+'&chs='+chs+'&r='+Math.random();
    }    
 }
...
?>   
 ...
 <div id="metabox_basic_settings" class="postbox" >
  ...
  <div class="inside">
     <form name="updatereportsettings" action="options-general.php?page=<?php echo $this->menu_parameter; ?>" method="post">
     ...
     <table class="form-table">    
        ...
        <td><textarea type="text" name="CP_POLLS_rep_message" rows="3" cols="80"><?php echo esc_attr(get_option('CP_POLLS_rep_message', 'Attached you will find the data from the form submissions.')); ?></textarea></td>
        </tr>        
        <tr valign="top">
        <th scope="row"></th>
...
[<a href="https://wordpress.org/support/plugin/cp-polls#new-post" target="_blank">Support</a>] | [<a href="<?php echo $this->plugin_URL; ?>" target="_blank">Documentation</a>]
</form>
</div>